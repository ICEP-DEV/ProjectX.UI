...
import { FaExclamationCircle } from 'react-icons/fa'; // Unused FaRegSmile removed for cleanup
...
const AlumniCommunity = () => {
  ...
  const handleSearch = () => {
    ...
    setShowNoResultsPopup(filtered.length === 0);
    ...
    <div className={`alumni-community ${isModalOpen ? 'blur-background' : ''}`}>
        ...
        <div className="container">
          <div className="row">
            <div className="col-lg-8 col-12 mx-auto text-center">
              <h1 className="text-black">Connect. Inspire. Celebrate.</h1>
              <h6>A Hub for TUT Graduates</h6>
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="search-container">
        ...
        <input 
          ...
          placeholder="Search by Name, Surname, Student Number, or Graduation Year..." 
          value={searchInput} 
      ...
      {isModalOpen && selectedAlumnus && (
        <div className="modal-overlay">
          <div className="modal">
            ...
            <h1>{`${selectedAlumnus.firstName} ${selectedAlumnus.lastName}`}</h1>
            {/* Modal Details */}
          </div>
        </div>
      )}
      ...
      {/* Popups */}
      {showNoResultsPopup && (
        <Popup closePopup={closePopup} message="No alumni found. Please refine your search." />
      )}
      {showEmptySearchPopup && (
        <Popup closePopup={closePopup} message="Task bar empty. Please enter a search term." />
      )}
    </div>
  );
};
